...
export default function MenuItemPriceProps({ name, addLabel, props, setProps }) {
    ...
    return (
        <div className="bg-gray-200 p-2 rounded-2xl mb-2">
            ...
            <div className={isOpen ? 'block' : 'hidden'}>
                {props?.length > 0 && props.map((size, index) => (
                    <div key={index} className="flex gap-2 items-end">
                        ...
                        <div>
                            <button type="button" onClick={() => removeProp(index)} className="bg-white mb-2 p-2"><Trash /></button>
                        </div>
                    </div>
                ))}
                <button onClick={addProp} type="button" className="bg-white"><Plus />
                    <span>{addLabel}</span>
                </button>
            </div>
        </div>
